document.addEventListener('DOMContentLoaded', () => {
    console.log('스크립트 초기화 완료. Nirvana Pokedex v6.0-final');
    ...
    function renderCalendarView(contentDiv, data, year, month) {
        ...
        calendarContainer.innerHTML = `
            ...
            <div class="calendar-legend">
                <div class="legend-item"><div class="legend-color-box" style="background-color: #FF4500;"></div><span>랭킹뽑기</span></div>
                <div class="legend-item"><div class="legend-color-box" style="background-color: #1E90FF;"></div><span>한정뽑기</span></div>
                <div class="legend-item"><div class="legend-color-box" style="background-color: #32CD32;"></div><span>복냥이</span></div>
            </div>
        ...
        app.addEventListener('click', e => {
            ...
            if (dateCell && !dateCell.classList.contains('other-month')) {
                ...
                (gachaData.events || []).forEach(event => {
                    const startDate = new Date(event.startDate + 'T00:00:00');
                    const endDate = new Date(event.endDate + 'T00:00:00');
                    if(clickedDate >= startDate && clickedDate <= endDate) { dayEvents.push(event); }
                });
                ...
                (gachaData.recurringEvents || []).forEach(event => {
                    ...
                    while(currentDate < stopDate) {
                        ...
                        if(startDate > clickedDate && startDate > stopDate) break;
                        currentDate.setDate(currentDate.getDate() + event.recurrence.interval * 7);
                    }
                });
