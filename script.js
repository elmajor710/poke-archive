// [모바일 전용 버전] Nirvana Pokedex script.js - 수정 버전
document.addEventListener('DOMContentLoaded', () => {
    console.log('모바일 전용 스크립트 초기화 완료');

    // ========================================
    // 갤럭시 네비게이션바 뒤로가기 기능 개선
    // ========================================
    ...
    function handleBackNavigation() {
        ...
        const lev4Panel = document.getElementById('lev4-panel');
        ...
        // 모달이 열려있으면 모달만 닫기
        const openModal = document.querySelector('.modal-overlay.custom-modal');
        ...
        if (listPage && listPage.classList.contains('visible')) {
            ...
            if (sessionStorage.getItem('returnToMain')) {
                ...
                lev4Panel.classList.remove('visible');
                if (lev3Panel.classList.contains('visible')) {
                    lev3Panel.classList.add('visible');
        ...
        } else if (mainPlaceholder.style.display !== 'none') {
            // 메인 화면에서는 브라우저 기본 뒤로가기 (앱 종료)
            history.back();
            isNavigating = false;
            return;
        }
    ...
    function showListPage(menuId, subMenuId = null) {
        history.pushState({ page: 'list', menuId, subMenuId }, '');
    ...
    function renderListPage(data, menuId) {
        ...
        renderCardList(data, menuId, listContent, 3);
    }
    ...
    function showDetailPage(itemId, menuId) {
        history.pushState({ page: 'detail', itemId, menuId }, '');
...
});// [모바일 전용 버전] Nirvana Pokedex script.js - 갤럭시 네비게이션바 뒤로가기 지원
document.addEventListener('DOMContentLoaded', () => {
    console.log('모바일 전용 스크립트 초기화 완료');

    // ========================================
    // 갤럭시 네비게이션바 뒤로가기 기능 구현
    // ========================================
    let navigationStack = []; // 현재 네비게이션 상태를 추적
    ...
    function handleBackNavigation() {
        if (navigationStack.length > 0) {
            ...
            if (listPage && listPage.classList.contains('visible')) {
                ...
                // 메인 화면에서는 뒤로가기 시 앱 종료 (기본 동작)
                if (navigationStack.length === 0) {
                    history.back(); // 실제 브라우저 뒤로가기
                }
            }
        } else {
            // 스택이 비어있으면 브라우저 기본 동작
            history.back();
        }
    }
    ...
    async function fetchAndRenderPopularDecks() {
        // 모바일에서는 사용하지 않음 (메인 화면이 그리드 버튼만 표시)
    }
    ...
    function handleMenuClick(button) {
        ...
        pushNavigationState({ level: parseInt(button.dataset.level), id: button.dataset.id });
    ...
    function showListPage(menuId, subMenuId = null) {
        pushNavigationState({ page: 'list', menuId, subMenuId });
    ...
    function showDetailPage(itemId, menuId) {
        pushNavigationState({ page: 'detail', itemId, menuId });
    ...
    function addEventListeners() {
        if(mobileMenuBtn && sidebar) {
            mobileMenuBtn.addEventListener('click', () => {
                sidebar.classList.toggle('visible');
                if (sidebar.classList.contains('visible')) {
                    pushNavigationState({ page: 'sidebar' });
                }
            });
        }
